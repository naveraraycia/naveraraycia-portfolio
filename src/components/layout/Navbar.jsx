...
function Navbar() {
  ...
  return (
    ...
    <ul id="menu" onClick={toggleMenu} className={`text-white fixed z-[99] top-0 bottom-0 left-0 hidden flex-col w-full min-h-screen items-center font-sans space-y-5 uppercase text-2xl bg-black bg-opacity-70 lg:hidden` }>
      ...
      <li onClick={() => handleClickScroll('portfolio')} className={`${darkMode === true ? 'hover:text-secondary' : 'hover:text-primary'} hover:cursor-pointer`}>Portfolio</li>
      <li onClick={() => handleClickScroll('contact')} className={`${darkMode === true ? 'hover:text-secondary' : 'hover:text-primary'} hover:cursor-pointer`}>Contact</li>
    </ul>
    </>
  )
}
